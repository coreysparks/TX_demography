...
p2<-txsp%>%
  ...
  #facet_wrap(~date, nrow=7)+
  labs(caption = "Source: TWC Unemployment Insurance Claims \n Calculations by Corey S. Sparks, Ph.D.")+
  ggtitle(label ="Per capita Unemployment Insurance Claims", subtitle="March 7, 2020 to September 25, 2021")

library(transformr)

anim <- p2 + 
  transition_time(time = itime)+
  labs(title = "Week: {closest_state}") 

animate(anim, 
        height = 600, width = 600)

# anim_save(filename = "Map animation.gif",
#           animation = anim3_p_nofw_map)
#     shadow_wake(wake_length = .5)

# anim

anim_save("txui.gif", anim)
